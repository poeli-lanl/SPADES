...
def build_payload(state: Dict[str, Any], base_dir: Optional[Path] = None) -> Dict[str, Any]:
    ...
    available_levels = sorted({item["level"] for item in pathogen_list})
    default_levels = [level for level in DEFAULT_LEVELS if level in available_levels]
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    h1 { margin:5px 0 0; font-size:clamp(1.65rem,3vw,2.35rem); line-height:1.08; }
    ...
    .history-table-wrap { width:100%; margin-top:14px; overflow-x:auto; border:1px solid var(--line); border-radius:9px; background:#fff; }
    .history-table { width:100%; min-width:760px; border-collapse:collapse; table-layout:auto; font-size:.78rem; }
    .history-table th { background:#f5f8fa; color:var(--muted); font-size:.67rem; font-weight:800; letter-spacing:.045em; padding:9px 12px; text-align:left; text-transform:uppercase; white-space:nowrap; }
    .history-table td { border-top:1px solid var(--line); padding:9px 12px; font-variant-numeric:tabular-nums; white-space:nowrap; }
...
<main id="app" class="app-shell" v-cloak>
  ...
  <section class="panel">
    ...
    <p-data-table v-model:expanded-rows="expandedRows" v-model:selection="selectedRows" :value="filteredTaxa" data-key="key" state-storage="local" state-key="spades-stream-results-v2" selection-mode="multiple" :meta-key-selection="false" paginator :rows="25" :rows-per-page-options="[10,25,50,100]" striped-rows removable-sort @row-expand="onRowExpand" @row-collapse="onRowCollapse">
      ...
      <template #expansion="slot"><div class="history"><div class="history-head"><div><strong>{{ slot.data.name }} metric history</strong><br><span>Missing values remain gaps; charts update in place with each cumulative result.</span></div><span>{{ slot.data.history.length }} observations</span></div><div class="chart-grid"><history-chart title="READ_COUNT" field="read_count" color="#197278" :history="slot.data.history" format="integer"></history-chart><history-chart title="BEST_SIG_COV" field="best_sig_cov" color="#3f7cac" :history="slot.data.history" format="percent"></history-chart><history-chart title="SNI_SCORE" field="sni_score" color="#d95d39" :history="slot.data.history" format="decimal"></history-chart></div><div class="history-table-wrap"><table class="history-table"><thead><tr><th>Batch</th><th>Completed</th><th>READ_COUNT</th><th>BEST_SIG_COV</th><th>SNI_SCORE</th></tr></thead><tbody><tr v-for="point in slot.data.history" :key="point.timepoint"><td><strong>B{{ point.timepoint }}</strong></td><td>{{ formatTime(point.completed_at) }}</td><td>{{ formatNullable(point.read_count) }}</td><td>{{ formatCoverage(point.best_sig_cov) }}</td><td>{{ formatSni(point.sni_score) }}</td></tr></tbody></table></div></div></template>
    </p-data-table>
  </section>
...
window.browserResourcesReady.then(function(){
  ...
  const app=createApp({components:{ChangePill,HistoryChart},setup(){
    ...
    const expansionStorageKey='spades-stream-expansion-v3';let expansionState={expanded:{},choices:{},pathogens_seen:{}};try{const saved=JSON.parse(localStorage.getItem(expansionStorageKey)||'{}');if(saved&&typeof saved==='object')expansionState={expanded:saved.expanded&&typeof saved.expanded==='object'?saved.expanded:{},choices:saved.choices&&typeof saved.choices==='object'?saved.choices:{},pathogens_seen:saved.pathogens_seen&&typeof saved.pathogens_seen==='object'?saved.pathogens_seen:{}}}catch(error){}
    ...
    function syncPathogenExpansion(taxa){const expanded={...expansionState.expanded};for(const taxon of taxa){if(!taxon.human_pathogen||expansionState.pathogens_seen[taxon.key])continue;expansionState.pathogens_seen[taxon.key]=true;if(!Object.prototype.hasOwnProperty.call(expansionState.choices,taxon.key))expanded[taxon.key]=true}for(const [key,choice] of Object.entries(expansionState.choices)){if(choice)expanded[key]=true;else delete expanded[key]}expansionState.expanded=expanded;restoreCanonicalExpansion();persistExpansion()}
    ...
    syncPathogenExpansion(totalLatestTaxa.value);
    ...
    async function applyPayload(next){if(next.generated_at===payload.value.generated_at)return;const selectedKeys=new Set(selectedRows.value.map(item=>item.key)),snapshot=captureScroll();syncPathogenExpansion(next.pathogens.filter(item=>item.present_latest));payload.value=next;selectedRows.value=next.pathogens.filter(item=>item.present_latest&&selectedKeys.has(item.key));await nextTick();restoreCanonicalExpansion();restoreScroll(snapshot);liveMessage.value='updated'}
